...
-- Add a '/' to the config path so that sourced files can be find-tabbed
nvimrc = vim.fn.stdpath 'config' .. '/'
vim.cmd('let $BASH_ENV = "/etc/profile"')
...
require('lazy').setup({
  ...
  'wfxr/minimap.vim',        -- Minimap (right-pane)
...
-- Amythest colors for root, else Emerald
if os.getenv("USER") == 'root' then
  vim.cmd('source ' .. nvimrc .. 'color/Amythest.vim')
else
  vim.cmd('source ' .. nvimrc .. 'color/Emerald.vim')
end

-- vim.cmd('source ' .. nvimrc .. 'color/Emerald.vim')   -- Use Emerald Legacy Color Scheme
vim.cmd('source ' .. nvimrc .. 'legacy/netrw.vim')    -- Enable Legacy Netrw config until I refactor
vim.cmd('source ' .. nvimrc .. 'legacy/brackets.vim') -- Enable Legacy brackets config until I refactor
...
set.updatetime    = 250
setw.wrap         = false               -- Line Wrapping off
setw.number       = true                -- Line Numbers default
setw.signcolumn   = 'yes'               -- Keep signcolumn on by default
...
vim.keymap.set('n', '<leader>/', function() -- See `:help telescope.builtin`
...
require('nvim-treesitter.configs').setup {
  ...
  ensure_installed = { 'c', 'cpp', 'go', 'javascript', 'lua', 'python', 'rust', 'ruby', 'tsx', 'typescript', 'vimdoc',
    'vim', 'yaml' },
